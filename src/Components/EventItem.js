            ...
            <div className="card my-3" style={{ width: "80vw" }}>
                <div className="card-body my-2">
                    <div className="d-flex align-items-baseline">
                        ...
                        {text !== "View Result" && role !== "P" ? <><i className="fas fa-edit mx-2" onClick={() => { editEvent(USEvent.E_ID, "Kumar", "Sanu ki awaz me", "Bollywood", "12:43:59", "2023-12-03", "1234567809") }}
                    ...
                    <p className="card-text mx-2">Location: {USEvent.Location} &emsp; Date: {Moment(USEvent.Date).format('YYYY-MM-DD')} &emsp;
                        Time: {USEvent.Time} {role!=='P' && text==='View Result'?<button type="button" style={{ position: "absolute", right: "2em" }} disabled={disabled} onClick={() => { handleClick(); }} className="btn btn-success">{textAlter}</button>:
                        role==='P'?<button type="button" style={{ position: "absolute", right: "2em" }} disabled={disabled} onClick={() => { handleClick(); }} className="btn btn-success">{textAlter}</button>:<></>}
